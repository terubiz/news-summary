...
/**
 * Discord通知送信アダプタ。
 * Discord Webhook URL に HTTP POST でメッセージを送信する。
 */
@Component
class DiscordNotificationAdapter(
    ...
    override fun send(
        ...
        val webhookUrl = config["webhookUrl"] ?: throw IllegalArgumentException("Discord Webhook URLが設定されていません")

        val message = NotificationMessageBuilder.buildPlainText(summary, impacts, sourceUrls)

        // Discord Webhook は content フィールドに最大2000文字
        val truncatedMessage = if (message.length > 2000) message.take(1997) + "..." else message
        val payload = mapOf("content" to truncatedMessage)
        ...
        logger.info("Discord送信完了: summaryId=${summary.id.value}")
    }
